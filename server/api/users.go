...
func ListSSLCerts(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		w.Header().Add("Content-type", "text/plain")
		http.Error(w, "Could not read certificates from trust store", http.StatusInternalServerError)
		return
	}
	keystore, err := getKeystore("keystore")
	if err != nil {
		w.Header().Add("Content-type", "text/plain")
		http.Error(w, "Could not get path to key store from the broker", http.StatusInternalServerError)
	}
...
func ImportSSLCert(w http.ResponseWriter, r *http.Request) {
	...
	if err = truststore.ImportCert(alias, cert); err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] api.ImportSSLCert: %s", err)
